...
module Operators =
    ...
    [<Test>]
    [<TestCase("riPepperonis", 10)>]
    [<TestCase("KKona", 4)>]
    [<TestCase("MorphinTime", 2)>]
    [<TestCase("TwitchSings", 21)>]
    [<TestCase("LUL", 73)>]
    let BinaryTest (operator:string)(value:int) =
        let exitCode = runSource $"SabaPing TheIlluminati {operator} Kappa" 
